...
data, meta_data = ts.get_intraday(symbol='AA', interval='1min', outputsize='full')
...
def main():
    ...
    high_data, low_data = get_stock_intraday(ts, open, 1, 0)
    print(high_data)
    print(low_data)

def get_stock_intraday (ts, open, interval, ct):
    data, meta_data = ts.get_intraday(symbol='AA', interval='1min', outputsize='full')
    data.index = pd.DatetimeIndex(data.index) + timedelta(hours=3, minutes=58)
    ...
    while is_date:
        ...
        if len(result_data) == 0:
            ...
            percent_0_5_percent_1 = len(
                result_data.where((result_data['2. high'] > percent_0_5) & (result_data['2. high'] <= percent_1)).dropna())
            percent_1_percent_1_5 = len(
                result_data.where((result_data['2. high'] > percent_1) & (result_data['2. high'] <= percent_1_5)).dropna())
            percent_1_5_percent_2 = len(
                result_data.where((result_data['2. high'] > percent_1_5) & (result_data['2. high'] <= percent_2)).dropna())
            ...
            high = [open_percent_0_5, percent_0_5_percent_1, percent_1_percent_1_5, percent_1_5_percent_2, percent_2_over]
            ...
            print('low')
            print(open_percent_m_0_2, percent_m_0_2_percent_m_0_3, percent_m_0_3_percent_m_0_4, percent_m_0_4_percent_m_0_5,
                  percent_m_0_5_less)
